...
Cost: $65.85 total ($0.095/question). Answer score is token F1 for categories 1-4 and adversarial detection accuracy for category 5. Token F1 is a diagnostic metric — it penalizes verbosity (gold answers average ~4 tokens; system answers are 30-40 tokens). LLM-as-judge accuracy is the primary answer quality metric (58.8% on 759 questions). Both metrics are now reported automatically on every benchmark run.
...
Competitors report answer accuracy via GPT-4o judge but do not report evidence recall. Flywheel reports evidence recall, LLM-as-judge accuracy (primary), and token F1 (diagnostic).
