...
export function registerQueryTools(
  ...
  server.tool(
    'search',
    'Search everything — notes, entities, and memories — in one call. Returns a decision surface: note results (with section provenance, full section content up to 2500 chars, dates, bridges, confidence), matching entity profiles, and relevant memories. Top results carry full metadata (frontmatter, scored backlinks/outlinks, snippets). Start with just a query, no filters. Narrow with filters only if needed. Between snippet, section_content, and frontmatter, most questions can be answered without follow-up reads. Searches note content (FTS5 plus hybrid semantic with contextual embeddings), entity profiles, and stored memories. In multi-vault mode, omitting vault searches all vaults and merges results.',
    {
