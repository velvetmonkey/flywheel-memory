...
/**
 * Register schema intelligence tools
 */
export function registerSchemaTools(
  ...
  server.registerTool(
    'infer_folder_conventions',
    {
      ...
      description:
        'Auto-detect metadata conventions for a folder. Analyzes field frequency, types, common values, and naming patterns. Returns inferred schema with confidence scores.',
      inputSchema: {
        folder: z.string().optional().describe('Folder to analyze (e.g., "meetings/"). If omitted, analyzes entire vault.'),
        min_confidence: z.coerce.number().min(0).max(1).optional().describe('Minimum confidence threshold (0.0-1.0). Default 0.5.'),
      },
    },
  ...
  server.registerTool(
    'find_incomplete_notes',
    {
      ...
      description:
        'Find notes missing expected fields based on inferred folder conventions. Returns completeness scores and suggested values.',
      inputSchema: {
        folder: z.string().optional().describe('Folder to analyze. If omitted, analyzes entire vault.'),
        min_frequency: z.coerce.number().min(0).max(1).optional().describe('Minimum field frequency to consider "expected" (default 0.7).'),
        limit: z.coerce.number().optional().describe('Maximum results to return (default 50).'),
        offset: z.coerce.number().optional().describe('Number of results to skip for pagination (default 0).'),
      },
    },
  ...
  server.registerTool(
    'suggest_field_values',
    {
      ...
      description:
        'Suggest appropriate values for a frontmatter field based on vault usage. Context-aware suggestions based on folder and existing fields.',
      inputSchema: {
        field: z.string().describe('Field name to get suggestions for'),
        folder: z.string().optional().describe('Limit suggestions to this folder'),
        existing_frontmatter: z.record(z.unknown()).optional().describe('Other frontmatter fields in the note for context'),
      },
    },
