...
1. **Pre-warm** (1 Claude Haiku session): `doctor(action: health)` -> `refresh_index` -> `init_semantic` (embeddings) -> `doctor(action: health)`. This matches production usage -- Flywheel indexes and embeds the vault before questions start. (Auto-wikilinks are applied on write, so they accrue naturally as the vault is populated.)
...
1. **Pre-warm** (1 Claude Haiku session with `full,memory` preset): `doctor(action: health)` -> `refresh_index` -> `init_semantic` (embeddings) -> `doctor(action: health)`.
